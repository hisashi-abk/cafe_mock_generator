...
class CafeDataGenerator:
    ...
    def generate_customers(self, num_customers: int = 500) -> pd.DataFrame:
        ...
        for i in range(num_customers):
            ...
            gender = np.random.choice(
                ["male", "female"],
                p=[customer_config["gender_distribution"]["male"], customer_config["gender_distribution"]["female"]],
            )
    ...
    def generate_orders_and_items(
        self, customers_df: pd.DataFrame, menu_df: pd.DataFrame
    ...
    def _select_menu_items(
        self, customer: pd.Series, available_menu: pd.DataFrame, order_date: datetime
    ) -> Dict[int, int]:
    ...
    def generate_daily_summary(self, orders_df: pd.DataFrame, order_items_df: pd.DataFrame) -> pd.DataFrame:
        ...
        order_summary = orders_df.merge(
            order_items_df.groupby("order_id").agg({"subtotal": "sum", "quantity": "sum"}).reset_index(), on="order_id"
        )

        # 日次でグループ化
        daily_summary = (
            order_summary.groupby(order_summary["order_datetime"].dt.date)
            .agg(
                {
                    "order_id": "count",  # 注文数
    ...
    def _inject_noise(self, orders_df: pd.DataFrame, order_items_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if missing_rate > 0:
            # 注文データの一部を欠損にする
            missing_indices = np.random.choice(orders_df.index, size=int(len(orders_df) * missing_rate), replace=False)
        ...
        if outlier_rate > 0:
            outlier_indices = np.random.choice(
                order_items_df.index, size=int(len(order_items_df) * outlier_rate), replace=False
            )
            # 価格を10倍にして異常値とする
